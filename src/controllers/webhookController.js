...
const { reprocessProductImages } = require("./mediaController");
...
exports.handleProductUpdate = async (req, res) => {
  try {
    ...
    if (webhookData?.Notifications?.[0]?.Data?.[0]) {
      ...
      if (productError) {
        console.error(
          "❌ Error fetching product from kv_products:",
      ...
      if (!oldProduct) {
        console.warn(
          `⚠️ Product with KiotViet ID ${kiotvietProductId} not found in kv_products.`
        );
        return res.status(200).json({
      ...
      if (productDbId && branchId) {
        const { data: oldInventory, error: inventoryError } =
          await getRecordByField(
        ...
        if (inventoryError) {
          console.error(
            `❌ Error fetching inventory for product ID ${productDbId}, branch ${branchId}:`,
            inventoryError.message
          );
          // Continue processing product changes, but log inventory fetch error
        } else if (!oldInventory) {
          console.warn(
            `⚠️ Inventory not found for product ID ${productDbId}, branch ${branchId}.`
          );
        } else {
        ...
        console.warn(
          "⚠️ Skipping inventory check: productDbId or branchId missing."
        );
      }
      ...
      if (changes.length > 0) {
        ...
        if (changes.some((c) => c.field === "baseprice"))
          productUpdateData.base_price = parseFloat(productUpdate.BasePrice);
        if (changes.some((c) => c.field === "description"))
          productUpdateData.description = productUpdate.Description;
        ...
        if (
          ...
          if (updateInventoryError)
            console.error(
              "❌ Error updating kv_product_inventories:",
              updateInventoryError.message
            );
          else console.log("✅ kv_product_inventories updated.");
        }

        // 5. Reprocess product images if price or cost changed
        let imagesReprocessed = false;
        if (
          changes.some((c) => c.field === "baseprice" || c.field === "cost")
        ) {
          try {
            console.log(
              "🔄 Triggering image reprocessing for updated product..."
            );
            imagesReprocessed = await reprocessProductImages(kiotvietProductId);
            if (imagesReprocessed) {
              console.log("✅ Product images reprocessed with new price data.");
            } else {
              console.log("⚠️ Product image reprocessing skipped or failed.");
            }
          } catch (reprocessError) {
            console.error(
              "❌ Error reprocessing product images:",
              reprocessError
            );
            // Continue with the response - image reprocessing is optional
          }
        }
        ...
        return res.status(200).json({
          ...
          images_reprocessed: imagesReprocessed,
        });
      } else {
    ...
    console.warn(
      "⚠️ Webhook received but no valid product notification data found."
    );
    return res.status(200).json({
