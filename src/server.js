      ...
      try {
        ...
        try {
          ...
          const response = await axios.get(
            `http://localhost:${PORT}/media/price-table/generate`
          );
          console.log(
            "✅ Price table image generation request successful:",
            response.data && response.data.message
              ? response.data.message
              : "No message from endpoint."
          );
        } catch (genError) {
          console.error(
            "❌ Error triggering price table image generation:",
            genError.message
          );
          if (genError.response) {
            console.error("Error response data:", genError.response.data);
            console.error("Error response status:", genError.response.status);
          }
        }
      } catch (err) {
        console.error(
